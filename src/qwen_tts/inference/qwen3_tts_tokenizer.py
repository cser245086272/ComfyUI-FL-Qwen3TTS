...
class Qwen3TTSTokenizer:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, **kwargs) -> "Qwen3TTSTokenizer":
        ...
        # Extract device from kwargs - it's not a valid arg for AutoModel.from_pretrained
        ...
        # Move model to target device if specified
        if target_device is not None:
            inst.model = inst.model.to(target_device)
            inst.device = torch.device(target_device)
        else:
