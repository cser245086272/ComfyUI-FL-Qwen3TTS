...
class TTSDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        for i,data in enumerate(batch):
            ...
            # NOTE: Position 6 is where speaker embedding is inserted during training.
            # We keep it True (not masked) to match the working implementation.
            # The speaker embedding overwrites whatever is at position 6 anyway.

            codec_mask[i,   8+text_ids_len-1:8+text_ids_len-1+codec_ids_len] = True
            attention_mask[i, :8+text_ids_len+codec_ids_len] = True
        
        # Concatenate ref_mels - matches official implementation
        # When using consistent ref_audio (recommended), all ref_mels have same shape
