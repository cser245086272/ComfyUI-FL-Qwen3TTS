...
class Qwen3TTSTalkerForConditionalGeneration(Qwen3TTSTalkerTextPreTrainedModel, GenerationMixin):
    ...
    def _update_model_kwargs_for_generation(self, outputs, model_kwargs, is_encoder_decoder=False, num_new_tokens=1):
        ...
        if hasattr(self, '_progress_callback') and self._progress_callback is not None:
            max_tokens = getattr(self, '_max_new_tokens', None) or model_kwargs.get('max_new_tokens', 2048)
            current_step = outputs.generation_step if outputs.generation_step is not None else 0
            self._progress_callback("generating", current_step, max_tokens)
...
class Qwen3TTSForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        # Set progress callback on talker for per-token updates
        if progress_callback is not None:
            self.talker._progress_callback = progress_callback
            self.talker._max_new_tokens = max_new_tokens
        ...
        # Clear progress callback after generation
        if progress_callback is not None:
            self.talker._progress_callback = None
            self.talker._max_new_tokens = None
